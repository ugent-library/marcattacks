...
if (!FIXES['to_rdf']) {
    // Idiomatic Fix shape: a positional path followed by named options.
    //   to_rdf(.)                          blank nodes relabelled unique/record
    //   to_rdf(., skolem:'<iri-prefix>')   blank nodes skolemized to IRIs
    // The path is always the whole record, so it is ignored. `skolem` is parsed
    // as a named option (key then value, as marc_map does). The prefix MUST be
    // quoted — the ':' in https:// is an argument separator to the Fix parser.
    FIXES['to_rdf'] = (args: any[]) => {
        const a = args || [];
        const si = a.indexOf('skolem');
        const skolem = (si >= 0 && typeof a[si + 1] === 'string') ? a[si + 1] : undefined;
